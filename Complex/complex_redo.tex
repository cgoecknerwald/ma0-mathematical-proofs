...
\firstpageheader{Claire Goeckner-Wald}{}{25 Aug 2015}
...
\par
Now, consider the second case that $z$ has some imaginary part not equal to $0$. Then in Cartesian representation, $z = a + bi$, we know that $b \neq 0$. Thus, by the definition of modulus, $|z| = |a+bi| = \sqrt{a^2 + b^2}$. We know that $\textrm{Re}(z) = a$. However, it is not true that $a = \sqrt{a^2 + b^2}$, because by some simple algebraic manipulations, we see that this would require the real number $b$ to be $0$. We begin by attempting to show that
\begin{equation*}
a = \sqrt{a^2 + b^2}.
\end{equation*}
...
\par
Let us begin with $p \Rightarrow q$ by proof of contrapositive $\neg q \Rightarrow \neg p$. Written, the contrapositive states ``if $\textrm{Im}(z) \neq 0$ and $\textrm{Re}(z) \neq 0$, then $\left(\overline z\right)^2 \neq z^2$". As such, consider a $z$ contained in the set of complex numbers $ \mathbb{C}$. We will represent $z$ in Cartesian form as $z = a + bi$ where $\textrm{Im}(z) \neq 0$ and $\textrm{Re}(z) \neq 0$. Recall that in this scenario, $\textrm{Im}(z) =b$ and $\textrm{Re}(z) = a$. Thus $a\neq 0$ and $b \neq 0$. Then, we have that
\begin{equation*}
...
\end{equation*}
The initial statement ``if $\textrm{Im}(z) \neq 0$ and $\textrm{Re}(z) \neq 0$, then $\left(\overline z\right)^2 \neq z^2$" asks us to show that  $\left(\overline z\right)^2 $ and $z^2$ are inequivalent. Thus, we have
\begin{align*}
